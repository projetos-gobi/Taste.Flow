...
import { CreateCategoryTypeModal } from "@/src/components/create-category-type-modal"
import { EditCategoryTypeModal } from "@/src/components/edit-category-type-modal"
import useSession from "@/src/hooks/useSession"
import { getCategoryTypesPaged, softDeleteCategoryType } from "@/src/services/category-type"
import { toast } from "sonner"
import { useCategoryTypeCreateModal, useCategoryTypeEditModal } from "@/src/hooks/useModal"
...
export default function TipoCategoriaPage() {
  ...
  const fetchInitialData = async (page: number = 1) => {
    try {
      ...
      const response = await getCategoryTypesPaged(data);
  ...
  const handleDeleteCategoryType = async (id: string) => {
    if (confirm("Tem certeza que deseja excluir esse tipo de categoria?")) {
      setIsLoading(true);
      session.setRefresh(false);

      try {
        const response = await softDeleteCategoryType({ id: id});
    ...
    <div className="bg-gray-50 p-6 min-h-full relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <Button
            onClick={() => categoryTypeCreateModal.openModal(true)}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => categoryTypeEditModal.openModal(true, categoryType.id)}
      ...
      <CreateCategoryTypeModal isOpen={categoryTypeCreateModal.isOpen}/>

      <EditCategoryTypeModal isOpen={categoryTypeEditModal.isModalOpen}/>
    </div>
  )
}
