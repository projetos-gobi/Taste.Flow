...
import { Package, Edit, Trash2, Plus, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Loader2 } from "lucide-react"
import { CreateTypeModal } from "@/src/components/create-type-modal"
import { EditTypeModal } from "@/src/components/edit-type-modal"
import useSession from "@/src/hooks/useSession"
import { toast } from "sonner"
import { getProductTypesPaged, softDeleteProductType } from "@/src/services/product-type"
import { useProductTypeCreateModal, useProductTypeEditModal } from "@/src/hooks/useModal"
...
export default function TipoPage() {
  ...
  const fetchInitialData = async (page: number = 1) => {
    try {
      ...
      const response = await getProductTypesPaged(data);
  ...
  const handleDeleteProductType = async (id: string) => {
    if (confirm("Tem certeza que deseja excluir esse tipo?")) {
      ...
      try {
        const response = await softDeleteProductType({ id: id});
        ...
        console.error("Erro ao deletar um tipo:", error)
      } finally {
    ...
    <div className="bg-gray-50 p-6 min-h-full relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <Button
            onClick={() => productTypeCreateModal.openModal(true)}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => productTypeEditModal.openModal(true, productType.id)}
      ...
      {/* Modais */}
      <CreateTypeModal isOpen={productTypeCreateModal.isOpen} />

      <EditTypeModal isOpen={productTypeEditModal.isModalOpen} />
    </div>
  )
}
