...
import { CreateCategoryModal } from "@/src/components/create-category-modal"
import { EditCategoryModal } from "@/src/components/edit-category-modal"
import useSession from "@/src/hooks/useSession"
import { getCategoriesPaged, softDeleteCategory } from "@/src/services/category"
import { toast } from "sonner"
import { useCategoryCreateModal, useCategoryEditModal } from "@/src/hooks/useModal"
...
export default function CategoriasPage() {
  ...
  const fetchInitialData = async (page: number = 1) => {
    try {
      ...
      const response = await getCategoriesPaged(data);
  ...
  const handleDeleteCategory = async (id: string) => {
    if (confirm("Tem certeza que deseja excluir essa categoria?")) {
      ...
      try {
        const response = await softDeleteCategory({ id: id});
    ...
    <div className="bg-gray-50 p-6 min-h-full relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <Button
            onClick={() => categoryCreateModal.openModal(true)}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => categoryEditModal.openModal(true, category.id)}
      ...
      <CreateCategoryModal isOpen={categoryCreateModal.isOpen} />

      <EditCategoryModal isOpen={categoryEditModal.isModalOpen}/>
    </div>
  )
}
