...
import { useStockEntryCreateModal } from "../hooks/useModal"
...
import { ICreateStockEntryRequest, StockEntry } from "../types/stock-entry"
...
import { createStockEntry } from "../services/stock-entry"
import { Checkbox } from "./ui/checkbox"
import { CreateStockEntryAttachment, StockEntryAttachment } from "../types/stock-entry-attachment"
...
export function CreateEntryControlModal({ isOpen }: CreateEntryControlModalProps) {
  ...
  const fetchInitialData = async (page: number = 1) => {
    try {
      const [categoriesResponse, unitsResponse, merchandisesResponse, paymentTypesResponse, suppliersResponse, paymentTermsResponse] = await Promise.all([
  ...
  useEffect(() => {
    if(stockEntryCreateModal.isOpen){
      fetchInitialData();
    }
  }, [stockEntryCreateModal.isOpen]);
  ...
  const calculateTotalLaunchedValue = () => {
    ...
    const itemTotal = Number.parseFloat(removeCurrencyMask(item.totalAmount)) || 0
  ...
  const fileToStockEntryAttachment = async (file: File): Promise<CreateStockEntryAttachment> => {
    const base64 = await new Promise<string>((resolve, reject) => {
    const reader = new FileReader()
    reader.onload = () => resolve((reader.result as string).split(",")[1]) // tira o prefixo "data:..."
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const stockEntryAttachments = await Promise.all(
        attachedFiles.map(fileToStockEntryAttachment)
      );

      const request: ICreateStockEntryRequest = {
        ...stockEntry,
        ...
        stockEntryAttachments: stockEntryAttachments 
      };
      
      const response = await createStockEntry(request);
  ...
  const handleClose = () => {
    ...
    stockEntryCreateModal.openModal(false);
  };
    ...
    <>
      <Dialog open={isOpen} onOpenChange={handleClose}>
        <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <p className="font-bebas text-3xl text-gray-900 tracking-wide">Cadastro de Entradas</p>
            <DialogTitle className="hidden"></DialogTitle>
            <p className="text-muted-foreground font-nunito text-gray-600">Cadastrando uma nova leva de entradas de Mercadoria.</p>
          </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
              ...
              <div className="space-y-2">
                ...
                <AutocompleteField
                  ...
                  className="border border-gray-300 rounded-md bg-white font-nunito font-medium"
                />
              </div>
