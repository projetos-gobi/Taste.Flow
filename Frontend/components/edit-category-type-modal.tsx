...
interface EditCategoryTypeModalProps {
  ...
  categoryType: { id: number; name: string } | null
  onEditCategoryType: (id: number, name: string) => void
}

export function EditCategoryTypeModal({
  open,
  onOpenChange,
  categoryType,
  onEditCategoryType,
}: EditCategoryTypeModalProps) {
  ...
  const handleConfirm = () => {
    if (categoryType && name.trim()) {
      onEditCategoryType(categoryType.id, name.trim())
      onOpenChange(false)
    }
  }
