...
const FilePreviewModal: React.FC<{
  ...
  const isExistingFile = (file as any).isExisting
  const fileSize = isExistingFile ? (file as any).originalSize : file.size

  const handleDownload = () => {
    if (isExistingFile) {
      alert("Download de arquivos existentes será implementado com a integração do backend.")
      return
    }
        ...
        <DialogHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="font-nunito text-sm text-gray-500">{(fileSize / 1024 / 1024).toFixed(2)} MB</p>
            </div>
...
export function EditEntryControlModal({ isOpen, onClose, onSave, entry }: EditEntryControlModalProps) {
  ...
  // Carregar dados da entrada quando o modal abrir
  useEffect(() => {
    if (entry && isOpen) {
      setFormData({
        supplier: entry.supplier || "",
        purchaseDate: entry.purchaseDate || "",
        expectedReceiptDate: entry.expectedReceiptDate || "",
        responsible: entry.responsible || "",
        nfNumber: entry.nfNumber || "",
        paymentMethod: entry.paymentMethod || "",
        paymentTerm: entry.paymentTerm || "",
        totalNfValue: entry.totalNfValue || "",
      })

      setMerchandiseItems(
        entry.merchandiseItems || [
          {
            id: 1,
            merchandise: "",
            category: "",
      ...
      // Converter anexos existentes para objetos File simulados
      const existingFiles = (entry.attachedFiles || []).map((fileData: any) => {
        // Criar um objeto File simulado para arquivos existentes
        ...
        Object.defineProperty(file, "originalSize", { value: fileData.size, writable: false })
        return file
      })
  ...
  const openFilePreview = (file: File) => {
    // Para arquivos existentes, mostrar uma mensagem informativa
    if ((file as any).isExisting) {
      alert("Preview não disponível para arquivos existentes. Use o botão de download para baixar o arquivo.")
      return
    }
    setPreviewFile(file)
    ...
    <>
      <Dialog open={isOpen} onOpenChange={handleClose}>
        <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl font-bebas text-gray-900">Editar Entrada</DialogTitle>
